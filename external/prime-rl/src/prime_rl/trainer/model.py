...
try:  # pragma: no cover - optional dependency only required when using Liger kernels
...
def get_model(
    ...
    with device:
        match config.impl:
            ...
            case "liger_kernel":
                if AutoLigerKernelForCausalLM is None:  # pragma: no cover
                    raise ImportError(
                        "Model implementation 'liger_kernel' requested but `liger-kernel` is not installed. "
                        "Install `liger-kernel` or choose `--model.impl=hf`."
                    )
